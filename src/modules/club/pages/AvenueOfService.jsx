...
function ProjectsGoalCard() {
  const [entries, setEntries]       = useState([])   // user-added { title, target, avenueLabel, avenueColor }
  const [query, setQuery]           = useState('')
  const [picked, setPicked]         = useState(null)
  const [count, setCount]           = useState('')
  const [open, setOpen]             = useState(false)
  const [editTitle, setEditTitle]   = useState(null) // title currently in edit mode
  ...
  // Auto-include projects that were completed but never added to the goal
  const userTitles = new Set(entries.map(e => e.title))
  const autoEntries = PROJECT_OPTIONS
    .filter(o => completedCountFor(o.title) > 0 && !userTitles.has(o.title))
    .map(o => {
      const cnt = completedCountFor(o.title)
      return {
        title:       o.title,
        target:      cnt,
  ...
  const totalCompleted = displayEntries.reduce((s, e) => s + Math.min(completedCountFor(e.title), e.target), 0)
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 px-5 py-4 relative">
      ...
      {/* Entries list — user-added + auto-included completed projects */}
      {displayEntries.length === 0 ? (
...
/* ── Project Completion Frequency card ─────────────────────────── */
function ProjectFrequencyCard() {
  const map = new Map()
  PROJECT_CATALOG.forEach(p => {
    const cur = map.get(p.title) || { title: p.title, avenueLabel: p.avenueLabel, avenueColor: p.avenueColor, count: 0 }
    cur.count++
    map.set(p.title, cur)
  })
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200">
      <div className="px-5 py-3 border-b border-slate-100">
        ...
        <p className="text-xs text-slate-500 mt-0.5">How many times each project has been done this RY</p>
      </div>
